...
let rewardFactor = 0.00000001; // Changed from miningFactor to rewardFactor
let adViewingInterval; // Changed from miningInterval to adViewingInterval
let dashboardSyncInterval;

function startAdViewing(email) { // Changed from startMining to startAdViewing
  fetch(`https://danoski-backend-hc8i.onrender.com/user/dashboard?email=${encodeURIComponent(email)}`)
    ...
    .then(data => {
      ...
      dashboardSyncInterval = setInterval(() => {
        syncEarnedPoints(email).then(() => { // Changed from syncMinedBTC to syncEarnedPoints
...
function syncEarnedPoints(email) { // Changed from syncMinedBTC to syncEarnedPoints
...
function watchAd() { // New function for watching ads
  const email = sessionStorage.getItem("email");
  if (!email) return;
...
function loadActiveCampaigns() { // Changed from loadActiveHashrates to loadActiveCampaigns
  const email = sessionStorage.getItem("email");
  ...
  fetch(`https://danoski-backend-hc8i.onrender.com/user/hashrates?email=${encodeURIComponent(email)}`)
    ...
    .then(data => {
      ...
      data.forEach((item, index) => {
        ...
        li.innerHTML = `<b>${item.hashrate} ads/hr</b> - <span id="${id}">Calculating...</span>`; // Display as ads per hour
        list.appendChild(li);
