...
const rewardFactor = 0.00000001 // Changed from miningFactor
let adViewingInterval // Changed from miningInterval
let dashboardSyncInterval

function startAdViewing(email) {
  // Changed from startMining
  fetch(`https://danoski-backend-hc8i.onrender.com/user/dashboard?email=${encodeURIComponent(email)}`)
    .then((res) => res.json())
    .then((data) => {
      let adViewRate = data.hashrate // Backend still uses hashrate
      let pointsBalance = data.btc_balance // Backend still uses btc_balance
      let lastViewed = new Date(data.last_mined).getTime() // Backend still uses last_mined
...
function watchAd() {
  ...
  document.getElementById("ad-status").innerText = "📺 Watching advertisement..." // Changed message
...
function loadActiveCampaigns() {
  // Changed from loadActiveHashrates
  const email = sessionStorage.getItem("email")
  ...
  fetch(`https://danoski-backend-hc8i.onrender.com/user/active-campaigns?email=${encodeURIComponent(email)}`) // Updated route
    ...
    .then((data) => {
      ...
      data.forEach((item, index) => {
        ...
        const id = `campaign-timer-${index}` // Changed from hashrate-timer
