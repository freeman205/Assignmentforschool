document.addEventListener("DOMContentLoaded", () => {
  ...
  // The accessToken variable here will be null if not logged in initially.
  // It's better to retrieve it inside the DOMContentLoaded listener or functions.
  // const accessToken = sessionStorage.getItem('accessToken'); // This line is less critical but can be misleading.
  ...
  // This block ensures functions are called only when the DOM is ready and accessToken is available
  document.addEventListener("DOMContentLoaded", () => {
    const accessToken = sessionStorage.getItem("accessToken")

    if (accessToken) {
      loadDashboardStats(accessToken) // ✅ Correctly called with accessToken
      loadActivity(accessToken) // ✅ Added this call here
    } else {
  ...
  async function loadProfile(accessToken) {
    // Ensure accessToken is used here as well
    try {
...
`
  ...
  // ❌ REMOVE THESE LINES: They cause the 401 error if accessToken is not ready.
  // loadDashboardStats();
  // loadActivity();
})
