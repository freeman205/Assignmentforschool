document.addEventListener("DOMContentLoaded", () => {
  ...
  // accessToken is now retrieved inside the DOMContentLoaded listener for initial calls,
  // and passed as an argument to functions that need it.
  const currentUser = JSON.parse(sessionStorage.getItem("currentUser")) // Used for transfer history display
  ...
  // This block ensures initial data loading happens only when the DOM is ready
  // and an accessToken is available.
  document.addEventListener("DOMContentLoaded", () => {
    const accessToken = sessionStorage.getItem("accessToken")

    if (accessToken) {
      loadDashboardStats(accessToken) // ✅ Called with accessToken
      loadActivity(accessToken) // ✅ Called with accessToken
    } else {
      ...
      actionSection.innerHTML = `
        <div class="bg-white p-6 rounded-lg shadow">
          ...
          <ul class="space-y-2">
            ${data.transfers
              .map((item) => {
                const isSender = currentUser && item.from_user.email === currentUser.email // Check currentUser
      ...
      document.getElementById("redeemForm").addEventListener("submit", async (e) => {
        ...
        try {
          ...
          if (r.ok) {
            ...
            // Refresh dashboard stats after successful redemption
  ...
  async function loadTransferForm(accessToken) {
    ...
    form.addEventListener("submit", async (e) => {
      ...
      try {
        ...
        if (typeof loadDashboardStats === "function") loadDashboardStats(accessToken) // Pass accessToken
      } catch (err) {
