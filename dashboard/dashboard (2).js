...
/**
 * Gets the authentication token from sessionStorage.
...
// Function to update user points and name in the header and overview
function updateHeaderAndOverviewPoints() {
  const currentUser = getCurrentUser()
  const userNameSpan = document.getElementById("userName")
  const userPointsSpan = document.getElementById("userPoints") // This ID is not in HTML, using pointsBalance
  const pointsBalanceSpan = document.getElementById("pointsBalance")
  const completedSurveysSpan = document.getElementById("completedSurveys")
  const totalEarnedSpan = document.getElementById("totalEarned")
  const pendingRedemptionsSpan = document.getElementById("pendingRedemptions")

  if (currentUser) {
    if (userNameSpan) userNameSpan.textContent = currentUser.name || "User"
    if (pointsBalanceSpan) pointsBalanceSpan.textContent = `${(currentUser.points_balance || 0).toFixed(2)} Points`
    if (completedSurveysSpan) completedSurveysSpan.textContent = `${currentUser.completed_surveys || 0} Surveys`
    if (totalEarnedSpan) totalEarnedSpan.textContent = `${(currentUser.total_earned_points || 0).toFixed(2)} Points`
    if (pendingRedemptionsSpan) pendingRedemptionsSpan.textContent = `${currentUser.pending_redemptions || 0} Requests`
  } else {
    if (userNameSpan) userNameSpan.textContent = "[User Name]"
    if (pointsBalanceSpan) pointsBalanceSpan.textContent = "0.00 Points"
    if (completedSurveysSpan) completedSurveysSpan.textContent = "0 Surveys"
    if (totalEarnedSpan) totalEarnedSpan.textContent = "0.00 Points"
    if (pendingRedemptionsSpan) pendingRedemptionsSpan.textContent = "0 Requests"
  }
}
...
async function loadSpecialOffers() {
  // IMPORTANT: Replace this mock data with actual API calls to fetch your special offers.
  // The 'image' URLs below are placeholders and should be replaced with URLs from your API.
  const mockOffers = [
    {
      id: 1,
      title: "Earn 700 SB",
      description: "Binance",
      image: "https://via.placeholder.com/64?text=BinanceOffer", // Placeholder: Replace with actual image URL from API
    ...
    },
    {
      ...
      image: "https://via.placeholder.com/64?text=CoinbaseOffer", // Placeholder: Replace with actual image URL from API
      disclaimer: "New users only.",
    },
  ]

  const offersContainer = document.getElementById("specialOffersList")
  // This section is not present in the provided HTML, but keeping the function for future use.
  // If you add a section with id="specialOffersList", this will populate it.
  if (offersContainer) {
  ...
  // The HTML provided does not have a "featuredWaysToEarnList"
...
async function loadShopPartners() {
  // IMPORTANT: Replace this mock data with actual API calls to fetch your shop partners.
  // The 'logo' URLs below are placeholders and should be replaced with URLs from your API.
  const mockPartners = [
    { id: 1, name: "Walmart", cashback: "1%", logo: "https://via.placeholder.com/64?text=WalmartLogo" }, // Placeholder
    { id: 2, name: "Best Buy", cashback: "Up to 4%", logo: "https://via.placeholder.com/64?text=BestBuyLogo" }, // Placeholder
    { id: 3, name: "Expedia", cashback: "Up to 3%", logo: "https://via.placeholder.com/64?text=ExpediaLogo" }, // Placeholder
    { id: 4, name: "eBay", cashback: "1%", logo: "https://via.placeholder.com/64?text=eBayLogo" }, // Placeholder
    { id: 5, name: "Lowe's", cashback: "10%", logo: "https://via.placeholder.com/64?text=LowesLogo" }, // Placeholder
    { id: 6, name: "Home Depot", cashback: "Up to 8%", logo: "https://via.placeholder.com/64?text=HomeDepotLogo" }, // Placeholder
  ]

  const partnersContainer = document.getElementById("shopPartnersList")
  // This section is not present in the provided HTML, but keeping the function for future use.
  // If you add a section with id="shopPartnersList", this will populate it.
...
async function loadGames() {
  // IMPORTANT: Replace this mock data with actual API calls to fetch your games.
  // The 'image' URLs below are placeholders and should be replaced with URLs from your API.
  const mockGames = [
    {
      id: 1,
      title: "Rock N' Cash",
      points: "13,224",
      image: "https://via.placeholder.com/80?text=Game1", // Placeholder
    ...
    },
    {
      ...
      points: "443",
      image: "https://via.placeholder.com/80?text=Game2", // Placeholder
    ...
    },
    {
      ...
      points: "37",
      image: "https://via.placeholder.com/80?text=Game3", // Placeholder
    ...
    },
    {
      ...
      points: "460",
      image: "https://via.placeholder.com/80?text=Game4", // Placeholder
  ...
  // This section is not present in the provided HTML, but keeping the function for future use.
  // If you add a section with id="gamesList", this will populate it.
  if (gamesContainer) {
...
async function handleTransferPoints(event) {
  ...
  try {
    ...
    updateHeaderAndOverviewPoints() // Update points in header and overview
  } catch (error) {
...
async function loadUserProfile() {
  ...
  if (currentUser) {
    ...
    // Fallback if currentUser is not in sessionStorage or incomplete
    try {
      const userProfile = await apiCall("/profile") // Assuming a /profile endpoint
      sessionStorage.setItem("currentUser", JSON.stringify(userProfile)) // Update sessionStorage
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  console.log("Current accessToken in sessionStorage:", accessToken)
  ...
  if (!accessToken) {
    ...
    console.log("accessToken found. Initializing dashboard.")
  }

  // Initialize dashboard components if on dashboard.html
  if (window.location.pathname.includes("dashboard.html")) {
    updateHeaderAndOverviewPoints()
    loadMoreWaysToEarnTasks()
    loadSpecialOffers() // Note: HTML does not have elements for these, but function is ready
    loadShopPartners() // Note: HTML does not have elements for these, but function is ready
    loadGames() // Note: HTML does not have elements for these, but function is ready
    loadSurveys()
    loadTransferHistory()
    ...
    // The HTML does not have a closeSidebarButton, so we'll rely on overlay click
    ...
    // Event listeners for dashboard action buttons (Take a Survey, Transfer Points, Redeem Points)
    document.querySelectorAll(".btn-primary[onclick]").forEach((button) => {
      const targetId = button.getAttribute("onclick").match(/#([a-zA-Z0-9-]+)/)?.[1]
      if (targetId) {
        ...
        button.addEventListener("click", () => {
          ...
          if (targetSection) {
            ...
            const sidebarLink = document.querySelector(`aside nav a[href="#${targetId}"]`)
            if (sidebarLink) {
              sidebarLink.click() // Simulate click to activate link and show section
            }
          }
        })
      }
    ...
    // Event listeners for Reset PIN and Change Password buttons
    const resetPinButton = document.getElementById("resetPinButton")
    if (resetPinButton) {
      resetPinButton.addEventListener("click", () => {
        showToast("Reset PIN functionality not yet implemented.", "info")
        // You would typically redirect to a reset PIN page here
    ...
    if (changePasswordButton) {
      changePasswordButton.addEventListener("click", () => {
        showToast("Change Password functionality not yet implemented.", "info")
        // You would typically redirect to a change password page here
