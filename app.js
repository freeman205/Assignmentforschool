document.addEventListener("DOMContentLoaded", () => {
  ...
  async function apiCall(endpoint, method = "GET", data = null, requiresAuth = false) {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(
          responseData.detail || `API Error: ${response.status} ${response.statusText || "Unknown Error"}`,
        )
      }
      return responseData
    } catch (error) {
...
window.displayMessage = window.displayMessage || displayMessage
window.clearMessage = window.clearMessage || clearMessage
window.getAuthToken = window.getAuthToken || getAuthToken
window.getCurrentUser = window.getCurrentUser || getCurrentUser
window.clearAuthData = window.clearAuthData || clearAuthData
window.handleLogoutButton = window.handleLogoutButton || handleLogoutButton
